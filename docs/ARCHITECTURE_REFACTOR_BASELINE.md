...
准备阶段不修改生产代码路径，不引入命令、交互接口或 Coordinator，也不改变存储 schema、
插件 ID、统计口径和窗口行为。
...
阶段 A/C 的假交互测试以此表为断言来源。危险操作的自动回滚备份只能由现有 Repository 操作
在真正提交时创建，不能在预览或取消路径提前创建。
...
- 当前 61 项发布回归和新增架构护栏均通过；
- Release 构建保持 0 警告、0 错误；
- 生产代码与 0.9.8 行为一致；
- 阶段 A 首先建立可替换的强类型交互边界，再覆盖取消、拒绝、无效输入和失败路径；
