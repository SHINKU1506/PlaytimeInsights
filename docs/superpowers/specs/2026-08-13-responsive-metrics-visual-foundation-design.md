...
1. 候选列数限制在有效的 `MinColumns..MaxColumns`，并且不超过子元素数量；
2. 首先选择能让单元格宽度接近 `PreferredItemWidth`、同时不低于 `MinItemWidth` 的候选；
3. 若多个候选与推荐宽度距离相同，选择列数较多者，以减少纵向占用；
4. 若可用宽度小于一张最小卡片，保留一列并使用可用宽度，不生成负尺寸或横向重叠；
5. 若 Measure 宽度为正无穷，使用有效推荐宽度和最大列数计算期望尺寸，避免依赖无限宽度排列。
...
- 完整行从可用区域左侧开始；不完整末行在 `CenterIncompleteRow=true` 时整体居中；
